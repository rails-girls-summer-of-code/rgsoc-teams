...
RSpec.describe DateRange do
  ...
  describe '#compact' do
    let (:conference)  { FactoryGirl.build_stubbed(:conference) }
    subject { described_class.new(start_date: Date.parse('2015-12-31'), end_date: Date.parse('2015-12-31')) }
    ...
    context 'depending on date range' do
      ...
      context ' when multiple dates in same month' do
        subject { described_class.new(start_date: Date.parse('2015-12-30'), end_date: Date.parse('2015-12-31')) }
      ...
      context 'when multiple dates in different months' do
        subject { described_class.new(start_date: Date.parse('2015-11-30'), end_date: Date.parse('2015-12-01')) }
      ...
      context 'when multiple dates around New Year' do
        subject { described_class.new(start_date: Date.parse('2015-12-31'), end_date: Date.parse('2016-01-01')) }
      ...
      context 'when same months, different years' do
        subject { described_class.new(start_date: Date.parse('2015-12-01'), end_date: Date.parse('2016-12-31')) }
